...
Segments nuclei and quantifies mean/total RNAfish signal intensity per cell across all three channels. Transcriptional foci are detected using a watershed-based algorithm. Outputs `analysis1.csv`.

### `Image_2.ipynb` — IRF3 Nuclear Intensity Distribution
**Channels:** IRF3, DAPI  
**Groups:** Mock 24H, GS 24H, GS 32H, GS 48H  
Quantifies IRF3 nuclear intensity distribution by comparing mean IRF3 fluorescence inside each nucleus to a surrounding cytoplasmic ring. Classifies each nucleus as nuclear or non-nuclear. Outputs `analysis3.csv`.
...
Quantifies micronuclei frequency and IRF3 nuclear signal, stratifying cells by micronucleus status (MN+ vs MN−). Micronuclei are detected in the perinuclear region and filtered by morphology and IRF3 intensity. Outputs `analysis4.csv`.

### `Image_4.ipynb` — Cell Cycle Marker Intensity Quantification
**Channels:** IRF3, DAPI, Ki67  
**Groups:** Mock 32H, GS 32H  
Quantifies Ki67 cell cycle marker intensity per cell alongside IRF3 nuclear localization. Computes a per-nucleus Spearman correlation between DAPI and Ki67 intensities. Outputs `analysis6.csv`.
